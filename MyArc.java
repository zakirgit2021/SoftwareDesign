...
public class MyArc extends MyShape
{
    ...
    public MyArc(MyPoint p, int r, double startAngle, double angle, MyColor color)
    {
        this.center = p;
        this.radius = r;
    ...
    //area = angle/360 *piR^2
    public double area (){return (angle/360)*Math.PI*Math.pow(radius, 2);}
    public double perimeter(){return getArcLength();}
    @Override
    public String toString ()
    {   return "MyArc centered at ("+center.getX()+","+
                center.getY()+"), with radius "+radius +
                " Starting angle "+startAngle+" and angle "
                +angle+", with area "+this.area() +" and color "+color.getColor();
    }
